...
def get_models(G, model_names):
    MODELS = {'poisson':gl.ssl.poisson(G),  # poisson learning
             ...
             'beta0':beta_learning(G),
              ...
              'rwlldecay01':poisson_rw_laplace_decay_both(G, tau=0.01, tau_ll=0.01),
              'rwlldecayzero01': poisson_rw_laplace_decay_both(G, tau=0.0, tau_ll=0.01),
              'rwlldecay1':poisson_rw_laplace_decay_both(G, tau=0.1, tau_ll=0.1),
              'rwlldecayzero1': poisson_rw_laplace_decay_both(G, tau=0.0, tau_ll=0.1),
              'rwlldecay0100': poisson_rw_laplace_decay(G, tau=0.1),
              'rwlldecay0010': poisson_rw_laplace_decay(G, tau=0.01)
              }

    return [deepcopy(MODELS[name]) for name in model_names]

def get_poisson_weighting(G, train_ind, tau=0.0, normalization='combinatorial'):
    n = G.num_nodes
    F = np.zeros(n)
    F[train_ind] = 1
    if normalization == 'combinatorial':
        ...
        F -= np.mean(G.degree_matrix(p=0.5) * F) * G.degree_vector(p=0.5)
...
# this class has tau for both reweighting and for LL solve. 
class poisson_rw_laplace_decay_both(gl.ssl.ssl):
...
# This is the "proper version" of rwll decay.
class poisson_rw_laplace_decay(gl.ssl.ssl):
    def __init__(self, W=None, class_priors=None, tau=0.0, normalization='combinatorial', tol=1e-5, alpha=2, zeta=1e7, r=0.1):
        """Reweighted Laplace Learning with Decay
        ===================
    ...
    def _fit(self, train_ind, train_labels, all_labels=None):
        ...
        #Reweighting
        w = get_poisson_weighting(self.graph, train_ind) 
        D = sparse.spdiags(w, 0, w.size, w.size)
        G = gl.graph(D * self.graph.weight_matrix * D)
        ...
        A = L[idx,:]
        A = A[:,idx]
        A += self.tau*sparse.eye(A.shape[0]) # add the tau diagonal for Laplace Learning
